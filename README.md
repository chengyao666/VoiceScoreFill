...
通过语音输入学生姓名和成绩，在 **WPS 云端表格（网页端 Excel）** 或 **Google Sheets** 中自动定位单元格并填入分数的纯端侧方案。文档包含业务流程、技术架构、模块设计、关键数据结构、风险控制及测试策略，可直接作为立项或评审材料。
...
- 全流程在本地运行，无需服务器或额外权限。
- 不改动 WPS 或 Google Sheets 后台，也不依赖特权接口。
...
| 自动定位学生所在行 | 在姓名列查找匹配学生（WPS / Google Sheets） |
